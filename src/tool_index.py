...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_documents": "List, read, delete, restore, or tidy documents in the editor panel. action='list' returns clickable rows (most-recent first) so the user can open any doc by clicking; add deleted=true to list deleted docs instead. action='read' (aka view/open/get) with document_id returns the content; supports offset=<N> + limit=<N> to page through large docs (response includes next_offset when more remains, so you can keep calling with offset=next_offset). action='delete' with document_id removes a doc (only way to delete — always requires document_id, never guesses). Deletion is soft: action='restore' with the same document_id undoes it. Use this for ANY 'show/read/list/open/restore/undelete my documents/docs/files/notes' request — never shell or curl.",
